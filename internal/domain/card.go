...
// Card is LeetMate's view of a spaced-review card. The opaque scheduler state
// is stored serialized (by the review package) in FSRSState; LeetMate only needs
// the derived scheduling fields for the UI.
type Card struct {
	Slug         string
	FSRSState    []byte    // serialized scheduler state (JSON)
